...
async function processState(code, state, modal) {
  ...
  if (state.phase === "kraken") {
    ...
    const s3 = addLog(
      s2,
      `${p.name} draws ${SUIT_INFO[card.suit].emoji} ${card.suit}(${card.value})`,
    );
...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", ({ code, name }) => {
    ...
    console.log(
      `[room] ${code} started: "${room.names[0]}" vs "${room.names[1]}"`,
    );
  ...
  socket.on("player_draw", () => {
    ...
    if (!card) {
      processState(roomCode, { ...s, phase: "game_over" }, null);
      return;
    }
  ...
  socket.on("player_modal_select", ({ idx }) => {
    ...
    switch (modal.type) {
      case "cannon":
        result = resolveCannonSuit(s, modal.suits[idx]);
        break;
      case "sword":
        result = resolveSwordSuit(s, modal.suits[idx]);
        break;
      case "hook":
        result = resolveHookSuit(s, modal.suits[idx]);
        break;
      case "map":
        result = resolveMapCard(s, idx, modal.cards);
        break;
      default:
        return;
    }
...
http.listen(PORT, () =>
  console.log(`☠ Dead Man's Draw++ server running on :${PORT}`),
);
